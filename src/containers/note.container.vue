  ...
  <div class="component-box note shadow-sm">
    ...
    <div class="row m-0">
      <div class="title col-10 p-0">
        ...
        <textarea v-if="editTitle" class="form-control title-input" v-model="title" />
      </div>
      <div class="edit-button col-1 p-3 justify-content-center" @click="toggleEditTitle()">
        <font-awesome-icon v-if="!editTitle" icon="pencil-alt" class="edit-button-icon" />
        <font-awesome-icon v-if="editTitle" icon="times" class="edit-button-icon" />
      </div>
    </div>
  ...
  methods: {
    loadBreadcrumb() {
      this.breadcrumb = [
        ...
        {
          name: this.note.notebookName,
          link: `/collections/${this.note.notebookId}`,
        },
      ];
    },
    loadNote() {
